...
export interface CrossIndustryNudgeV1 {
  ...
  // 2026-08-17 신규 — industry_pain과 cross_industry_example 사이를 잇는 명시적
  // 연결 인사이트("사례가 뜬금없이 튀어나온다"는 관찰 기록 계기). 두 사실을 재서술
  // 하지 않고 공유하는 기술적/구조적 공통분모(메커니즘)를 1-2문장으로 명시 —
  // sharp vs generic insight 원칙(위 시장 검증 현황 참고)의 이 섹션 버전.
...
If context includes a "[Bank-specific financial data]" block, this company is a bank/lender — follow that block's instructions for key_metrics instead of the Revenue/Operating margin defaults above (it tells you to use Net Interest Income in place of Revenue, leave Operating margin "Not applicable", and base YoY growth on Net Interest Income).
...
{"industry_pain":{"title":"Pain point title, 8 words or fewer","description":["Pain point bullet common across the analyzed company's industry (by SIC/KSIC classification) as a whole — not specific to this one company, 1 line","2nd bullet, 1 line"],"financial_impact_question":"A question only — never a stated number, percentage, or dollar-figure conclusion — about how this pain point could hit the business financially. Example: 'How much of quarterly revenue could this bottleneck be putting at risk?'"},"connection_insight":"1-2 sentences naming the specific shared mechanism that makes cross_industry_example a real analog to industry_pain — not a restatement of either.","cross_industry_example":{"source_industry":"The OTHER industry this solution came from — must be genuinely different from the analyzed company's own industry","case_name":"The company or case name that solved it","solution_description":"How that other industry solved an analogous operational problem, 2 sentences max"},"key_bullets":["Why this pain point matters right now for this company, 8 words or fewer","The core mechanism behind the cross-industry solution, 8 words or fewer","The single biggest condition for applying this solution here, 8 words or fewer"],"sources":[{"index":1,"level":"L1","organization":"Source organization name","content":"Key point, 1 line","url":"https://..."}]}
Determine the analyzed company's industry (SIC/KSIC classification) from the provided context first. industry_pain.description: 2-4 short bullets, one distinct point per bullet, describing a pain point shared across that industry broadly, not a problem unique to this one company. cross_industry_example.source_industry must differ from the analyzed company's own industry — find a company in a different industry that solved a genuinely analogous operational problem. financial_impact_question must be phrased strictly as a question — never assert a number, percentage, or dollar-figure impact as fact. connection_insight is the bridge between industry_pain and cross_industry_example: you picked this particular cross-industry case because you already saw an analogy to industry_pain — now state that analogy explicitly by naming the concrete shared mechanism (the same underlying physical constraint, cost driver, regulatory pattern, or technical bottleneck both industries face), not by summarizing either fact again. A generic sentence like "both industries face similar challenges" is not acceptable — name the specific mechanism (e.g. "both are bottlenecked by the same weight-vs-energy-density tradeoff in battery chemistry"). This field synthesizes the two facts above, so it does not need its own new source citation or [n] marker. Both facts need at least one real, verifiable source URL in sources[].url — a real URL is required here (unlike other sections, don't leave it null); if you can't find a real source URL for a fact, drop that fact rather than inventing a URL. Tag industry_pain bullets and the solution_description with [n] source markers (bullets: marker at the end of the bullet; solution_description: marker at the end of the sentence it supports).`,
...
const SECTION_CONTENT_SIGNALS: Record<string, { text?: string[]; arrays?: string[] }> = {
  ...
  cross_industry_nudge_v1: { text: ['connection_insight'],                      arrays: ['key_bullets', 'sources'] },
