...
function buildContext(data: AnalysisDetail): string {
  ...
  const risks = fin?.key_risks?.slice(0, 3).join(' / ')
    ?? data.risks?.slice(0, 3).join(' / ')
    ?? '';
  ...
  const businessModel = bm
    ? `${bm.growth_motion} — ${bm.growth_motion_detail} (GM: ${bm.unit_economics.gross_margin}%, OPM: ${bm.unit_economics.operating_margin}%)`
    : data.business_model || '';

  const competitors = cv2
    ? `포지션: ${cv2.competitive_position}. 직접경쟁사: ${cv2.direct.slice(0, 3).map(c => `${c.name}(${c.country})`).join(', ')}`
    : data.competitors
    ? JSON.stringify(data.competitors).slice(0, 300)
    : '';
  ...
  return [
    ...
    risks ? `리스크: ${risks}` : '',
    financials ? `재무: ${financials.slice(0, 500)}` : '',
    businessModel ? `비즈니스 모델: ${businessModel.slice(0, 400)}` : '',
    competitors ? `경쟁사: ${competitors}` : '',
  ]
    .filter(Boolean)
    .join('\n\n');
}
